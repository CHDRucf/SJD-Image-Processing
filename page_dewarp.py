...
# from textCropping import text_crop, column_crop
...
TEXT_MIN_HEIGHT = 1      # min reduced px height of detected text contour
TEXT_MIN_ASPECT = 1    # filter out text contours below this w/h ratio
TEXT_MAX_THICKNESS = 50  # max reduced px thickness of detected text contour

EDGE_MAX_OVERLAP = 75.0   # max reduced px horiz. overlap of contours in span
EDGE_MAX_LENGTH = 75.0  # max reduced px length of edge connecting contours
EDGE_ANGLE_COST = 25.0   # cost of angles in edges (tradeoff vs. length)
EDGE_MAX_ANGLE = 50.0     # maximum change in angle allowed between contours
...
SPAN_MIN_WIDTH = 1      # minimum reduced px width for span
SPAN_PX_PER_STEP = 1    # reduced px spacing for sampling along spans
FOCAL_LENGTH = 1.2       # normalized focal length of camera

# TEXT_MIN_WIDTH = 2      # min reduced px width of detected text contour
# TEXT_MIN_HEIGHT = 2      # min reduced px height of detected text contour
# TEXT_MIN_ASPECT = 1    # filter out text contours below this w/h ratio
# TEXT_MAX_THICKNESS = 8  # max reduced px thickness of detected text contour
#
# EDGE_MAX_OVERLAP = 25.0   # max reduced px horiz. overlap of contours in span
# EDGE_MAX_LENGTH = 25.0  # max reduced px length of edge connecting contours
# EDGE_ANGLE_COST = 30.0   # cost of angles in edges (tradeoff vs. length)
# EDGE_MAX_ANGLE = 35.0     # maximum change in angle allowed between contours
#
# RVEC_IDX = slice(0, 3)   # index of rvec in params vector
# TVEC_IDX = slice(3, 6)   # index of tvec in params vector
# CUBIC_IDX = slice(6, 8)  # index of cubic slopes in params vector
...
# SPAN_PX_PER_STEP = 25    # reduced px spacing for sampling along spans
# FOCAL_LENGTH = 1.1
...
def resize_to_screen(src, maxw=1400, maxh=2300, copy=False):
...
def dewarp(img):
    ...
    # print(page_outline)
    cinfo_list = get_contours(img, small, pagemask, 'text')
    # print(cinfo_list)
    spans = assemble_spans(img, small, pagemask, cinfo_list)

    if len(spans) < 3:
        # print('  detecting lines because only', len(spans), 'text spans')
        cinfo_list = get_contours(img, small, pagemask, 'text')
        spans2 = assemble_spans(img, small, pagemask, cinfo_list)
    ...
    corners, ycoords, xcoords = keypoints_from_samples(img, small,
                                                       pagemask,
                                                       page_outline,
                                                       span_points)
    ...
    params = optimize_params(img, small,
                             dstpoints,
                             span_counts, params)
    ...
    # print('about to dewarp')
    outfile = remap_image('img', img, small, page_dims, params)
    # print("dewarped")
    return outfile
